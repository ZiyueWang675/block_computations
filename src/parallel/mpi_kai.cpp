...
int main(int argc, char* argv[])
{
    ...
    for (int n = 0; n < my_size; n++){
        for (int i = 0; i < local_stripes; i++){
            ...
            for (int i = 0; i < local_stripes; i++){
                col_number = (dimension / my_size) * ((my_rank+index)%my_size) + i;
                for (int k = 0; k < dimension; k++){
                    element(matrix_C, row_number, col_number) += element(matrix_A, row_number, k) * element(matrix_B, k, col_number);
                }
            }
        }
        index += 1;
