...
void copy(double *x /* out */, double *y /* in  */, int size /* in  */);
...
// The macro method to get the processor number based on the position of its
// initial block in the matrix
#define getID(row, col)                                                        \
  (col + sqrt_processors) % sqrt_processors +                                  \
      ((row + sqrt_processors) % sqrt_processors) * sqrt_processors
...
int main(int argc, char *argv[]) {
  ...
  if (dimension % sqrt_processors != 0) {
    if (my_rank == 0)
      printf("The dimension must be divisible by the square root of processors "
             "number!\n");
  ...
  if (my_rank == 0) {
    ...
    for (int i = 1; i < processors; i++) {
      MPI_Recv(local_C, block_number, MPI_DOUBLE, i, 1, MPI_COMM_WORLD,
               &status);
      ...
      for (int j = row_start; j < row_end; j++)
        for (int k = col_start; k < col_end; k++)
          element(matrixC, j, k) =
              local_C[(j - row_start) * block + k - col_start];
    }
    float end = MPI_Wtime();
    float read_time =
...
void fox() {
  MPI_Status status;
  // The position of the block need to send
  int local_col = row;
  int pos_start, pos_end;
  for (int stage = 0; stage < sqrt_processors; stage++) {
    if (col == local_col) {
      pos_start = getID(row, 0);
      pos_end = getID(row, sqrt_processors - 1);
      for (int i = pos_start; i <= pos_end; i++) {
        if (i == my_rank)
          continue;
        MPI_Send(local_A, block_number, MPI_DOUBLE, i, 1, MPI_COMM_WORLD);
      }
      copy(tempA, local_A, block_number);
    } else {
      MPI_Recv(tempA, block_number, MPI_DOUBLE, getID(row, local_col), 1,
    ...
    for (int i = 0; i < block; i++) {
      for (int j = 0; j < block; j++) {
        for (int k = 0; k < block; k++)
          local_C[i * block + j] +=
              tempA[i * block + k] * local_B[k * block + j];
      }
    }
    // Block in position (i,j) in B moved to (i - 1, j)
    MPI_Sendrecv(local_B, block_number, MPI_DOUBLE, getID(row - 1, col), 2,
                 tempB, block_number, MPI_DOUBLE, getID(row + 1, col), 2,
                 MPI_COMM_WORLD, &status);
