...
int main(int argc, char *argv[]) {
  ...
  if (my_rank == 0) {
    ...
    float end = MPI_Wtime();
    float read_time =
        (t2.tv_sec - t1.tv_sec) + (double)(t1.tv_usec - t2.tv_usec) / 1000000.0;
...
void cannon() {
  ...
  MPI_Sendrecv(local_B, block_number, MPI_DOUBLE, getID(row - col, col), 2,
               tempB, block_number, MPI_DOUBLE, getID(row + col, col), 2,
               MPI_COMM_WORLD, &status);
